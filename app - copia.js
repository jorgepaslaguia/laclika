...
(() => {
  ...
  const bindEvents = () => {
    ...
    const modeLibraryBtnEl = document.getElementById('mode-library');
    const modePrepBtnEl = document.getElementById('mode-prep');
    const modeKitchenBtnEl = document.getElementById('mode-kitchen');
    ...
    const handleValidationAction = (action, issueId = null) => {
      ...
      if (action === 'validation-add-resources') {
        ...
        planStatusEl.textContent = 'Recursos sugeridos agregados.';
        render();
        return;
      }
