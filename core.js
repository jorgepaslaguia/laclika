...
if (typeof window !== 'undefined') {
      ...
      const VERB_MAP = {
        ...
        BANO_MARIA: { phase: 'PREELABORACION', baseMin: 35, resourceKey: 'FOGONES' },
        ...
        REPOSAR: { phase: 'PREELABORACION', baseMin: 8, resourceKey: 'ESTACION' },
        ENFRIAR: { phase: 'PREELABORACION', baseMin: 10, resourceKey: 'ESTACION' }
      };
      ...
      const ENUMERATION_STRONG_RE = /^\s*(\d+)[\.\)]\s+(.+)$/;
      const DISH_HEADER_TOKENS = new Set([
        'menu',
        'entrantes',
        'entrante',
        'principales',
      ...
      const RESOURCE_CATALOG = [
        { id: 'HORNO', nombre: 'Horno', tipo: 'COCCION', pattern: 'horno(?:s)?', defaultCap: 1 },
        {
          ...
          nombre: 'Fogones',
          tipo: 'COCCION',
          pattern: 'fogones?|fogon(?:es)?|fuego|hornillo(?:s)?|cazuela(?:s)?|cacerola(?:s)?|olla(?:s)?|sarten(?:es)?|plancha',
          defaultCap: 3
        },
        {
          id: 'ESTACION',
          nombre: 'Estacion',
          tipo: 'PREP',
          pattern: 'estacion(?:es)?|mesa(?:s)?|tabla(?:s)?|cuchillo(?:s)?|encimera',
          defaultCap: 1
        },
        { id: 'FREGADERO', nombre: 'Fregadero', tipo: 'LIMPIEZA', pattern: 'fregadero(?:s)?|pila(?:s)?|lavadero', defaultCap: 1 },
      ...
      */
      ...
      const shouldLogUI = () => typeof window !== 'undefined' && window.DEBUG_UI;
        ...
        const state = {
          ...
          alertsOpen: false,
          issuesOpen: false,
          // User can ignore missing resources for the current plan.
      ...
      function normalizeResourceName(value) {
        ...
        if (/(fogon|fogones|cocina|hornillo|fuego|cazuela|cacerola|olla|sarten|plancha)/.test(normalized)) {
          return 'FOGONES';
        }
        if (/(estacion|mesa|tabla|preparad|cuchillo|encimera)/.test(normalized)) {
          return 'ESTACION';
        }
        if (/(fregadero|pila|lavadero)/.test(normalized)) {
          return 'FREGADERO';
        }
      ...
      function normalizePlan(inputPlan) {
        ...
        const planNormalized = {
          ...
          equipo: allowEmpty ? [] : team,
          recursos: allowEmpty ? [] : resources,
      ...
      function setPlan(nextPlan, draft = null, summary = '', questions = [], options = {}) {
        ...
        if (!options.keepIgnoreMissingResources) {
          state.ignoreMissingResources = false;
        }
      ...
      function mergeDishData(target, incoming) {
        ...
        merged.forcedEnumeration = merged.forcedEnumeration || incoming.forcedEnumeration || false;
      ...
      function mergeDishCandidate(dishes, candidate, threshold = DISH_SIMILARITY_THRESHOLD) {
        ...
        if (candidate.forcedEnumeration) {
          return null;
        }
        ...
        for (let i = 0; i < dishes.length; i += 1) {
          ...
          if (existing && existing.forcedEnumeration) {
            continue;
          }
...
function normalizeLine(line) {
      ...
      function splitInlineSectionDish(line) {
        ...
        const sectionKey = normalizeToken(section);
        if (
          /^(menu|recursos?|equipo|alergias?|ingredientes?|procesos?|observaciones?|notas?|pax|raciones|comensales)$/.test(
            sectionKey
          )
        ) {
          return null;
      ...
      function isMetadataLine(line) {
        ...
        if (/[,;:\-\u2022\u00b7]/.test(line) || ENUMERATION_STRONG_RE.test(line) || /\b\d{1,2}[\.\-)]\s+/.test(line)) {
          return false;
        }
        return true;
      }
      ...
      function isObviousDishHeader(text) {
        const normalized = normalizeToken(text || '');
        if (!normalized) {
          return false;
        }
        const tokens = normalized.split(' ').filter(Boolean);
        if (tokens.length === 1 && DISH_HEADER_TOKENS.has(tokens[0])) {
          return true;
        }
        return DISH_HEADER_TOKENS.has(normalized);
      }

      function inferForcedProcessesFromDishName(name) {
      ...
      function guessRoleLevel(value) {
        ...
        if (normalized.includes('chef') || normalized.includes('jefe')) {
          return { rol: 'CHEF', nivel: 3, label: 'Chef' };
        }
        if (normalized.includes('pinxe') || normalized.includes('pinche')) {
          return { rol: 'PINCHE', nivel: 2, label: 'Pinche' };
        }
        if (normalized.includes('ayudante') || normalized.includes('auxiliar')) {
          return { rol: 'AYUDANTE', nivel: 1, label: 'Ayudante' };
        }
        if (normalized.includes('cocinero')) {
          return { rol: 'COCINERO', nivel: 2, label: 'Cocinero' };
        }
        return { rol: 'EQUIPO', nivel: 1, label: 'Equipo' };
      }

      function parseTeamFromLines(lines) {
        const ROLE_PATTERNS = [
          { match: /chef|jefe/, rol: 'CHEF', label: 'Chef', nivel: 3 },
          { match: /cocinero/, rol: 'COCINERO', label: 'Cocinero', nivel: 2 },
          { match: /pinche|pinxe/, rol: 'PINCHE', label: 'Pinche', nivel: 2 },
          { match: /ayudante|auxiliar/, rol: 'AYUDANTE', label: 'Ayudante', nivel: 1 }
        ];
        const team = [];
        ...
        const makeUniqueName = (baseName) => {
          let name = baseName;
          let idx = 2;
          while (seenNames.has(normalizeToken(name))) {
            name = `${baseName} ${idx}`;
            idx += 1;
          }
          seenNames.add(normalizeToken(name));
          return name;
        };

        const nextRoleIndex = (role) => {
          const next = (roleCounters.get(role) || 0) + 1;
          roleCounters.set(role, next);
          return next;
        };

        const matchRole = (normalized) =>
          ROLE_PATTERNS.find((role) => role.match.test(normalized)) || null;

        const extractCount = (normalized) => {
          const cleaned = normalized.replace(/nivel\s*\d+/g, ' ').trim();
          const countFirst = cleaned.match(
            /(\d+)\s*(?:x\s*)?(chefs?|jefes?|cocineros?|pinches?|pinxes?|ayudantes?|auxiliares?)\b/
          );
          if (countFirst) {
            return sanitizeInt(countFirst[1], 1);
          }
          const countAfter = cleaned.match(
            /\b(chefs?|jefes?|cocineros?|pinches?|pinxes?|ayudantes?|auxiliares?)\b\s*[x*]\s*(\d+)/
          );
          if (countAfter) {
            return sanitizeInt(countAfter[2], 1);
          }
          return 1;
        };

        const addTeamMember = (name, roleInfo, rawItem) => {
        ...
        lines.forEach((rawLine) => {
          ...
          normalizedList
            ...
            .forEach((item) => {
              ...
              const normalizedItem = normalizeToken(rawItem);
              const roleInfo = matchRole(normalizedItem);
              if (roleInfo) {
                const count = Math.max(1, extractCount(normalizedItem));
                if (count > 1) {
                  for (let i = 0; i < count; i += 1) {
                    const index = nextRoleIndex(roleInfo.rol);
                    const name = makeUniqueName(`${roleInfo.label} ${index}`);
                    addTeamMember(name, roleInfo, rawItem);
                  }
                  return;
                }
                const nameCandidate = rawItem.split(/[\-|]/)[0].trim() || rawItem;
                const cleanedName = nameCandidate.replace(/\b\d+\b/g, '').trim();
                const normalizedName = normalizeToken(cleanedName);
                const isRoleOnly =
                  roleInfo.match.test(normalizedName) && normalizedName.split(' ').length <= 2;
                if (isRoleOnly) {
                  const index = nextRoleIndex(roleInfo.rol);
                  const roleLabel = index > 1 ? `${roleInfo.label} ${index}` : roleInfo.label;
                  const name = makeUniqueName(roleLabel);
                  addTeamMember(name, roleInfo, rawItem);
                  return;
                }
                if (isInvalidPersonName(cleanedName)) {
                  return;
                }
                const name = makeUniqueName(cleanedName);
                nextRoleIndex(roleInfo.rol);
                addTeamMember(name, roleInfo, rawItem);
                return;
              }
              ...
              const id = generatePersonId(usedIds);
              usedIds.add(id);
              const roleFallback = guessRoleLevel(rawItem);
              const levelMatch = rawItem.match(/nivel\s*(\d)/i);
              const levelValue = levelMatch ? clamp(parseInt(levelMatch[1], 10), 1, 3) : roleFallback.nivel;
              team.push({
                id,
                nombre: nameCandidate,
                rol: roleFallback.rol,
                nivel: levelValue,
                factor_velocidad: 1,
                restricciones: {}
              });
      ...
      function extractDishesFromLines(lines) {
        ...
        for (let i = start; i < endIndex; i += 1) {
          ...
          const match = String(line || '').match(ENUMERATION_STRONG_RE);
          if (match) {
            if (current) {
              blocks.push(current);
            }
      ...
      function parseDishBlock(blockLines, options = {}) {
        ...
        const nameOverride = options.nameOverride ? String(options.nameOverride) : '';
        const dishName = nameOverride
          ? sanitizeDishName(stripTimeFromLine(nameOverride))
          : cleanDishTitle(lines[0]);
        if (!dishName) {
          return null;
        }
        const firstLine = lines[0];
        const score = options.forceDish
          ? 1
          : scoreDishCandidate(firstLine, {
            dishName,
            isExplicitBlock: true,
            isNumbered: ENUMERATION_STRONG_RE.test(firstLine),
            isBullet: /^[\-\u2022\*]\s+/.test(firstLine),
            isCaps: isUppercaseLine(firstLine),
            hasTime: Boolean(extractTimeRange(firstLine))
          });
        if (!options.forceDish && score < DISH_SCORE_THRESHOLD) {
          return null;
        }
        ...
        const forcedProcesses = inferForcedProcessesFromDishName(dishName);
        forcedProcesses.forEach((process) => {
          if (!procesos.includes(process)) {
            procesos.push(process);
          }
        });
        ...
        return {
          ...
          pax,
          ingredientes,
          forcedEnumeration: Boolean(options.forceDish)
        };
      }
      function buildMenuIRFromRawText(rawText, sourceMeta = null) {
        ...
        const sectionHeaderOnlyRe = /^(entrantes?|principal(?:es)?|postres?|platos?|primeros?|segundos?)\s*[:\-\u2013\u2014]*$/i;
        ...
        const pushParsedDish = (parsed, blockLines) => {
          ...
          platos.push({
            ...
            forcedEnumeration: Boolean(parsed.forcedEnumeration)
          });
          if (parsed.forcedEnumeration) {
            diagnostics.forcedDishesFromEnumeration += 1;
            if (diagnostics.forcedEnumerationNames.length < 10) {
        ...
        if (!platos.length) {
          ...
          (parseLines || []).forEach((line) => {
            ...
            const inlineSection = splitInlineSectionDish(trimmed);
            if (inlineSection) {
              currentSection = normalizeSection(inlineSection.section);
              const rest = inlineSection.dish;
              const parsedInline = parseDishBlock([rest], {
                forceDish: true,
                nameOverride: rest
              });
              if (!parsedInline) {
              ...
              parsedInline.forcedEnumeration = false;
              parsedInline.fallbackLine = true;
              if (currentSection) {
                parsedInline.categoria = parsedInline.categoria || currentSection;
                parsedInline.section = parsedInline.section || currentSection;
              }
              pushParsedDish(parsedInline, [rest]);
              return;
            }
            ...
            const headerMatch = trimmed.match(sectionHeaderOnlyRe);
            if (headerMatch) {
            ...
            if (currentSection) {
              parsed.categoria = parsed.categoria || currentSection;
              parsed.section = parsed.section || currentSection;
            }
      ...
      function parseMenuDraft(text) {
        ...
        const fecha = detectDate(normalized);
          ...
          const isSectionHeaderLine = (raw) => {
            ...
            if (PROCESS_VERB_REGEX.test(normalizeToken(clean))) {
              return false;
            }
            ...
            const isUpper = isUppercaseLine(clean);
            const isTitle = words.every((word) => {
              const plain = stripDiacritics(word).replace(/[^A-Za-z]/g, '');
              if (!plain) {
                return false;
              }
              return plain[0] === plain[0].toUpperCase() && plain.slice(1) === plain.slice(1).toLowerCase();
            });
        ...
        const addDish = (name, time, origin, rawLine, context = {}) => {
          ...
          if (!context.forceDish && (dishName.length <= 3 || isIngredientDishName(dishName))) {
            return null;
          }
          const score = context.forceDish
            ? 1
            : scoreDishCandidate(rawLine || name, {
              dishName,
              ...context
            });
          if (!context.forceDish && score < DISH_SCORE_THRESHOLD) {
            return null;
          }
            const dish = {
              id: makeIdFromName(dishName, 'PLATO'),
              ...
              forcedEnumeration: Boolean(context.forceDish)
            };
          ...
          const forcedProcesses = inferForcedProcessesFromDishName(dishName);
          if (forcedProcesses.length) {
            dish.procesos = normalizeProcessList(dish.procesos.concat(forcedProcesses));
          }
        ...
        lines.forEach((rawLine) => {
          ...
          const menuMatch = rawLine.match(/^menu\b[^:]*:\s*(.*)$/i);
            ...
            if (inlineSection) {
              currentSection = normalizeSection(inlineSection.section);
              const rest = inlineSection.dish;
              const time = extractTimeRange(rest);
              addDish(rest, time, 'PDF', rest, {
                forceDish: true,
                isExplicitBlock: true,
                hasTime: Boolean(time)
              });
            ...
            if (isSectionHeaderLine(trimmed)) {
              currentSection = normalizeSection(trimmed.replace(/[:\-\u2013\u2014]+$/, '').trim());
              currentDish = null;
              mode = null;
              return;
            }
            const enumMatch = trimmed.match(ENUMERATION_STRONG_RE);
            if (enumMatch) {
              const enumeratedName = String(enumMatch[2] || '').trim();
              if (!isObviousDishHeader(enumeratedName)) {
              const time = extractTimeRange(enumeratedName);
              currentDish = addDish(enumeratedName, time, 'PDF', trimmed, {
                dishName: sanitizeDishName(stripTimeFromLine(enumeratedName)),
                cleanedLine: line,
                isNumbered: true,
                isBullet: false,
                isCaps: false,
          ...
          const isNumbered = ENUMERATION_STRONG_RE.test(trimmed) || /^\d{1,2}[\.\-)]\s+/.test(trimmed);
...
function buildTask(id, dishName, name, phase, duration, process, resource, level, deps, origin, confidence, resourceInfo = null) {
      ...
      function inferResourcesFromDraft(draft) {
        ...
        const processCounts = {};
        const hotProcesses = new Set(['COCER', 'HERVIR', 'SALTEAR', 'SOFREIR', 'FREIR', 'REDUCIR', 'BANO_MARIA']);
        const hornoProcesses = new Set(['HORNEAR', 'GRATINAR', 'ASAR']);
        const stationProcesses = new Set([
          'LAVAR',
          'ESCURRIR',
          'CORTAR',
          'MEZCLAR',
        ...
        dishes.forEach((dish) => {
          ...
          processes.forEach((process) => {
            ...
            if (hornoProcesses.has(process)) {
              typeCounts.HORNO += 1;
            }
            if (hotProcesses.has(process)) {
              typeCounts.FOGONES += 1;
            }
            if (stationProcesses.has(process)) {
              typeCounts.ESTACION += 1;
            }
            if (process === 'LAVAR' || process === 'ESCURRIR') {
              typeCounts.FREGADERO += 1;
            }
          });
        });
        if (!totalProcesses) {
        ...
        const hornoCap = typeCounts.HORNO > 0 ? 1 : 0;
        let fogonCap = 0;
        if (typeCounts.FOGONES > 0) {
          fogonCap = Math.max(1, Math.ceil(dishCount / 2));
        }
        if (processCounts.BANO_MARIA) {
          fogonCap = Math.max(fogonCap, 1);
        }
        fogonCap = Math.min(fogonCap, 6);
        let stationCap = typeCounts.ESTACION > 0 ? Math.max(1, Math.ceil(dishCount / 3)) : 0;
        stationCap = Math.min(stationCap, 4);
        const sinkCap = typeCounts.FREGADERO > 0 ? 1 : 0;

        if (hornoCap) {
          addResource(
            'HORNO',
            hornoCap,
            ['procesos: hornear/gratinar/asar'],
            clamp(0.65 + typeCounts.HORNO * 0.05, 0.65, 0.85)
          );
        }
        if (fogonCap) {
          addResource(
            'FOGONES',
            fogonCap,
            [`platos=${dishCount}`, 'procesos: cocer/hervir/saltear/freir/reducir'],
            clamp(0.65 + typeCounts.FOGONES * 0.04, 0.65, 0.85)
          );
        }
        if (stationCap) {
          addResource(
            'ESTACION',
            stationCap,
            [`platos=${dishCount}`, 'procesos: lavar/cortar/mezclar/emplatar'],
            clamp(0.6 + typeCounts.ESTACION * 0.04, 0.6, 0.85)
          );
        }
        if (sinkCap) {
          addResource(
            ...
            sinkCap,
            ['procesos: lavar'],
            clamp(0.6 + typeCounts.FREGADERO * 0.04, 0.6, 0.8)
          );
        }
        return resources;
      }
      ...
      function inferTeamFromDraft(draft, tasks, resources) {
        ...
        const pax = sanitizeInt(draft.comensales ?? draft.pax ?? null, 0);
        const evidence = [`tareas=${taskCount}`];
        if (pax) {
          evidence.push(`pax=${pax}`);
        }
        let pinches = Math.ceil(taskCount / 10);
        pinches = Math.min(pinches, 6);
        if (pax >= 20) {
          pinches += Math.floor(pax / 20);
        }
        pinches = Math.min(pinches, 8);
        const confidence = clamp(0.6 + evidence.length * 0.05, 0.6, 0.82);
        const ids = new Set();
        const team = [];
      ...
      function buildTasksFromProcessList(processList, dish, index, resources, suggestions) {
        ...
        let processes = normalizeProcessList(base.concat(explicitProcesses, extraProcesses));
          ...
          if (explicitProcesses.length) {
            // Avoid infinite recursion when explicit process lists are already normalized.
...
async function handlePdfFile(file) {
      ...
      function renderFilters() {
        ...
        function buildDiagnostics(planToCheck, errors, warnings, draft) {
          ...
          const addMissingType = (value) => {
            ...
            if (!missingResources.has(typeKey)) {
              missingResources.set(typeKey, RESOURCE_LABELS[typeKey] || typeKey);
            }
          };
          ...
          const ignoreMissing = Boolean(state && state.ignoreMissingResources);
          const missingResourceTypeKeys = ignoreMissing ? [] : Array.from(missingResources.keys());
          const missingResourceLabels = ignoreMissing ? [] : Array.from(missingResources.values());
          const diagnostics = {
            warnings: warnings || [],
            errors: errors || [],
            fixes,
            assumptions,
            missingResources: missingResourceLabels,
            missingResourceTypeKeys,
            missingDurations,
            unassignedTasks,
            inferredResources: (planToCheck?.recursos || []).filter(
              (resource) => normalizeSource(resource.source, 'explicit') === 'inferred'
            ),
            inferredTeam: (planToCheck?.equipo || []).filter(
              (person) => normalizeSource(person.source, 'explicit') === 'inferred'
            ),
            counts: {
        ...
        const summaryHtml = summaryItems
          .map((item) => {
            const fixBtn = item.fixAction
              ? `<button class="btn ghost mini-btn" type="button" data-action="${item.fixAction}">${item.fixLabel}</button>`
              : '';
            const reviewBtn = item.reviewAction
              ? `<button class="btn ghost mini-btn" type="button" data-action="${item.reviewAction}">${item.reviewLabel}</button>`
              : '';
            return `
        ...
        prepIssuesGridEl.innerHTML = cards
          .map(
            ...
            <div class="issue-card ${card.severity}">
              ...
              <button class="btn ghost mini-btn" type="button" data-action="${card.action}">${card.label}</button>
            </div>
          `
          )
          .join('');
      ...
      function renderLibrary() {
        ...
        function handleAlertAction(action) {
          ...
          if (action === 'add-missing-resources' || action === 'add-resources') {
            const created = addMissingResourcesFromDiagnostics();
            const assigned = assignResourcesByHeuristic({ silent: true });
            if (state?.debugEnabled) {
              console.debug('[alerts] add-missing-resources', { created, assigned });
            }
            render();
            return;
          }
      ...
      function getDefaultResourceCapacity(typeKey) {
        return 1;
      }

      function addMissingResourcesFromDiagnostics() {
        if (!plan) {
          return 0;
        }
        ...
        const diagnostics = state?.diagnostics || emptyDiagnostics();
        const missingTypeKeys = Array.isArray(diagnostics.missingResourceTypeKeys)
          ? diagnostics.missingResourceTypeKeys
          : [];
        const fallbackKeys = missingTypeKeys.length
          ? missingTypeKeys
          : (diagnostics.missingResources || []).map((label) => normalizeResourceName(label)).filter(Boolean);
        if (!fallbackKeys.length) {
          return 0;
        }
        const catalog = getEffectiveResourcesCatalog(plan.recursos);
        const existingIds = new Set(plan.recursos.map((resource) => resource.id));
        let created = 0;
        fallbackKeys.forEach((rawKey) => {
          const typeKey = normalizeResourceName(rawKey);
          if (!typeKey || !ALLOWED_RESOURCE_TYPES.has(typeKey)) {
            return;
          }
          if (catalog.byTypeKey[typeKey]?.length) {
            return;
          }
          const id = existingIds.has(typeKey) ? ensureUniqueId(typeKey, existingIds) : typeKey;
          existingIds.add(id);
          plan.recursos.push({
            id,
            nombre: RESOURCE_LABELS[typeKey] || typeKey,
            capacidad: getDefaultResourceCapacity(typeKey),
            tipo: inferResourceType(typeKey),
            typeKey,
            auto: true,
            suggested: true,
      ...
      function setIgnoreMissingResources(value) {
        state.ignoreMissingResources = Boolean(value);
        validateAndStore();
        calculatePlan({ silent: true });
        render();
      }
      ...
      function renderAssignments() {
        ...
        const isEmptyPlan = Boolean(plan?.isEmpty === true);
        const hasTasks = Boolean(plan && Array.isArray(plan.tareas) && plan.tareas.length);
        const isNoTasksPlan = !isEmptyPlan && !hasTasks;
        if (reviewSection) {
          reviewSection.classList.toggle('hidden', isEmptyPlan);
        }
        if (reviewEmptyEl) {
          reviewEmptyEl.hidden = !isNoTasksPlan;
        }
        if (isEmptyPlan || !hasTasks) {
          ...
          if (incompleteCountEl) {
            incompleteCountEl.textContent = isNoTasksPlan
              ? 'No se generaron tareas con este menu.'
              : 'Errores bloqueantes: 0 | Avisos: 0';
          }
          if (normalizedCountEl) {
            normalizedCountEl.textContent = isNoTasksPlan ? 'Revisa el formato o reintenta interpretar.' : '';
          }
          return;
        }
        ...
        plan.tareas.forEach((task) => {
          ...
          info.incomplete = errorTaskIds.has(task.id);
        ...
        if (incompleteCountEl) {
          incompleteCountEl.textContent = `Errores bloqueantes: ${errorCount} | Avisos: ${warningCount}`;
        }
        if (normalizedCountEl) {
      ...
      function getTaskIncompleteInfo(task) {
        ...
        return { incomplete: false, missing, expected, missingAny };
      }
...
function updateControls() {
      ...
      function render() {
        ...
        const hasPlanTasks = Boolean(plan && Array.isArray(plan.tareas) && plan.tareas.length > 0);
        const isPlanEmpty = Boolean(plan?.isEmpty === true);
        const isPlanNoTasks = !isPlanEmpty && !hasPlanTasks;
        document.body.classList.toggle('plan-empty', isPlanEmpty);
        document.body.classList.toggle('plan-no-tasks', isPlanNoTasks);
        document.body.classList.toggle('plan-has-tasks', hasPlanTasks);
        if (advancedPanelEl) {
      ...
      if (typeof window !== 'undefined') {
        ...
        window.addMissingResourcesFromDiagnostics = addMissingResourcesFromDiagnostics;
        window.setIgnoreMissingResources = setIgnoreMissingResources;
